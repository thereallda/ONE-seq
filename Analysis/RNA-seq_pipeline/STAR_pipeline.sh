...
help() {
		echo ""
		echo -e "Usage: \n\t bash $0 [options] --fastq1 <fastq1_file>  -d <project_dir> -o <output_dir> --ref <reference_genome> --gtf <GTF_flie>"
		echo ""
		echo -e "  -d \n\t\t Project directory."
...
if [[ ! -d ${OUTDIR}/star ]]; then mkdir -p ${OUTDIR}/star ; fi
ALIGNLOC=${OUTDIR}/star
...
if $PAIR; then
	...
	for ((j=0; j<=${#FQs[@]}-1; j+=2)); do
		...
		echo ""
		if [[ ! -d ${COUNTLOC}/${PREFIX}_counts ]]; then mkdir -p ${COUNTLOC}/${PREFIX}_counts; fi
		featureCounts \
		-p -B -C \
		-t exon \
		-g gene_id \
		-T ${THREADS} \
		-a $GTF \
		-o ${COUNTLOC}/${PREFIX}_counts/${PREFIX}_counts.txt \
...
multiqc -o ${OUTDIR}/star/  ${OUTDIR}/star/*/Log.final.out
