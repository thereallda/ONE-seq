...
1. `01_gtf_to_exon_saf.R`: Convert gtf file to simplified exon locations and save in saf format (also saves GENCODE gene information to txt).
2. `02_exon_saf_to_genebody_and_intron_saf.R`: Exon saf file is used to create genebody saf and intron saf files .
3. `03_genebody_saf_to_nonoverlapping_genebody_saf.R`: Create genebody saf file for non-overlapping genes.
4. `04_count_intron_reads.R`: Summarizing reads into intron counts.
5. `05_analysis_of_intron_counts.R`: Differential analysis on intron counts and visualization. 
