...
intron.avg$nad.y <- ifelse(intron.avg$GeneID %in% NAD_Ysig$GeneID, 'NAD-RNA', "Non-NAD-RNA")
intron.avg$nad.o <- ifelse(intron.avg$GeneID %in% NAD_Osig$GeneID, 'NAD-RNA', "Non-NAD-RNA")

# visualization of Intron TPM with scatter plot ----
## young ----
d <- table(intron.avg$Intron.stat.y, intron.avg$nad.y)
df1 <- reshape2::melt(t(t(d)/colSums(d)))
p1 <- ggplot(subset(intron.avg, GeneID %in% NAD_Ysig$GeneID), 
  ...
  labs(x='Log2(Intron TPM+1)\nInput', y='Enrichment\nLog2(Intron TPM+1)', 
       title='NAD-RNA', color='',
       subtitle=paste0('( % of Intron-enrichment = ', 
...
# old ---- 
d <- table(intron.avg$Intron.stat.o, intron.avg$nad.o)
df1 <- reshape2::melt(t(t(d)/colSums(d)))
p1 <- ggplot(subset(intron.avg, GeneID %in% NAD_Osig$GeneID), 
...
p2 <- ggplot(subset(intron.avg, !GeneID %in% NAD_Osig$GeneID), 
             aes(x=log2(O.In+1), y=log2(O.En+1))) +
  geom_point(aes(color=Intron.stat.o)) +
  geom_text(x=10, y=7, label='y = x') +
